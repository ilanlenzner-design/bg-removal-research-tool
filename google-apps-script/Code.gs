...
function scoreResult(e) {
  ...
  const prompt = `You are an EXTREMELY CRITICAL professional image quality inspector. Your reputation depends on finding even microscopic flaws. Examine this background removal result from ${payload.modelName} with a magnifying glass.
