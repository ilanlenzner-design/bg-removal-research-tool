// BG Compare Pro - Google Apps Script Backend (Detailed Column Storage)
...
// Model order for consistent column layout
const MODEL_ORDER = [
...
function getSheet() {
  ...
  if (!sheet) {
    ...
    // Initialize headers with detailed columns
    const headers = [
      'ID', 'Timestamp', 'Category', 'Name', 'Notes', 'Image Analysis',
      '851 Labs Model', '851 Labs Result URL', '851 Labs Overall Score',
      'Lucataco Model', 'Lucataco Result URL', 'Lucataco Overall Score',
      'BRIA AI Model', 'BRIA AI Result URL', 'BRIA AI Overall Score',
      'BiRefNet Model', 'BiRefNet Result URL', 'BiRefNet Overall Score',
      'RemBG Model', 'RemBG Result URL', 'RemBG Overall Score'
    ];
    ...
    sheet.setColumnWidth(6, 300); // Image Analysis
    // Model columns
    for (let i = 7; i <= headers.length; i += 3) {
      sheet.setColumnWidth(i, 120);     // Model Name
      sheet.setColumnWidth(i + 1, 350); // Model URL
      sheet.setColumnWidth(i + 2, 80);  // Model Score
    }
  }
...
function getTests() {
  ...
  for (let i = 1; i < data.length; i++) {
    try {
      // Reconstruct results and scores from columns
      const results = {};
      const scores = {};

      let colIndex = 6; // Start after Image Analysis
      MODEL_ORDER.forEach((modelId) => {
        const modelName = data[i][colIndex] || '';
        const modelUrl = data[i][colIndex + 1] || '';
        const modelScore = data[i][colIndex + 2] || '';
        ...
        if (modelScore) {
          try {
            scores[modelId] = JSON.parse(modelScore);
          } catch (e) {
            // Ignore parse errors
          }
        }
...
function createTest(e) {
  ...
  // Build row with detailed model columns
  const row = [
    id,
    timestamp,
    testData.category || '',
  ...
  // Add model results in order
  MODEL_ORDER.forEach((modelId) => {
    const result = testData.results ? testData.results[modelId] : null;
    const score = testData.scores ? testData.scores[modelId] : null;

    if (result) {
      row.push(result.name || '');
      row.push(result.output || '');
      row.push(score ? JSON.stringify(score) : '');
    } else {
      row.push('', '', '');
    }
  });
...
function updateTest(e, testId) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === testId) {
      ...
      // Add model results
      MODEL_ORDER.forEach((modelId) => {
        const result = updates.results ? updates.results[modelId] : null;
        const score = updates.scores ? updates.scores[modelId] : null;

        if (result) {
          updateRow.push(result.name || '');
          updateRow.push(result.output || '');
          updateRow.push(score ? JSON.stringify(score) : '');
        } else {
          updateRow.push('', '', '');
        }
      });

      // Update the row (starting from column 3 to preserve ID and timestamp)
      sheet.getRange(i + 1, 3, 1, updateRow.length).setValues([updateRow]);
...
// Replicate API proxy functions (same as before)
function getConfig() {
